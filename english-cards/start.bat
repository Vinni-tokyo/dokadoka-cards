@echo off
rem === 도카도카 영어 공부 / Dokadoka English — 로컬 서버로 열기 ===
rem YouTube 는 file:// 재생을 거부하므로(Error 153) 간이 서버를 띄워 http:// 로 연다.
setlocal
cd /d "%~dp0"
if /i "%CD:~0,2%"=="\\" goto :unc
if not exist "%~dp0English-Zendaya248-Cards.html" goto :nofile
set PORT=8096
set PAGE=English-Zendaya248-Cards.html
...
echo 서버를 시작합니다 ... %URL%
start "english-cards-server" /min %PY% -m http.server %PORT%
timeout /t 2 >nul
start "" "%URL%"
echo.
echo 최소화된 "english-cards-server" 창을 닫으면 서버가 종료됩니다.
timeout /t 3 >nul
exit /b 0
...
:nofile
echo [!] English-Zendaya248-Cards.html 을 찾을 수 없습니다.
