...
TITLES = {
    1: '出発進行', 2: '誤発進', 3: 'チハルとマキナ', 4: 'イートイン・ファイトアウト',
    5: '排除くん', 6: 'カートとマックス', 7: '仕事と見返り',
}
...
def build(ep):
    ...
    VID = VIDS[ep]
    OUT = os.path.join(ROOT, f'Japanese-Milky{ep}-Cards.html')
    ...
    tpl = tpl.replace(
        ...
        f'은하특급 밀키☆서브웨이 {ep}화 · <span class="n">—</span>문장<br><span lang="ja">銀河特急 ミルキー☆サブウェイ 第{ep}話「{TITLES[ep]}」· <span class="n">—</span>フレーズ')
    tpl = tpl.replace('japanese-yubisaki-edits.json', f'japanese-milky{ep}-edits.json')
    tpl = tpl.replace(
        '<title>도카도카 일본어 공부 · ドカドカ日本語</title>',
        f'<title>도카도카 일본어 공부 · 은하특급 밀키☆서브웨이 {ep}화</title>')

    html = (tpl
        .replace('/*__DATA__*/', json.dumps(data, ensure_ascii=False, separators=(',', ':')))
        .replace('/*__SCENES__*/', json.dumps([{'key': 'main', 'ko': f'{ep}화', 'ja': f'第{ep}話', 'at': 0}], ensure_ascii=False))
...
if __name__ == '__main__':
    eps = [int(a) for a in sys.argv[1:]] or list(range(1, 8))
    for ep in eps:
        build(ep)
