@echo off
rem === 도카도카 영어 공부 · 샘 올트먼 Bloomberg 인터뷰 / Dokadoka English — 로컬 서버로 열기 ===
rem YouTube 는 file:// 재생을 거부하므로(Error 153) 간이 서버를 띄워 http:// 로 연다.
setlocal enabledelayedexpansion
cd /d "%~dp0"
if /i "%CD:~0,2%"=="\\" goto :unc
if not exist "%~dp0English-Altman123-Cards.html" goto :nofile
set PORT=8095
set PAGE=English-Altman123-Cards.html
...
:nofile
echo [!] English-Altman123-Cards.html 을 찾을 수 없습니다.
