@echo off
rem === 도카도카 영어 공부 · 페이페이 리 The Circuit / Dokadoka English — 로컬 서버로 열기 ===
rem YouTube 는 file:// 재생을 거부하므로(Error 153) 간이 서버를 띄워 http:// 로 연다.
setlocal enabledelayedexpansion
cd /d "%~dp0"
if /i "%CD:~0,2%"=="\\" goto :unc
if not exist "%~dp0English-FeiFei250-Cards.html" goto :nofile
set PORT=8094
set PAGE=English-FeiFei250-Cards.html
...
:nofile
echo [!] English-FeiFei250-Cards.html 을 찾을 수 없습니다.
