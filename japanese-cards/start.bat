...
if not exist "%~dp0Japanese-SnowMan105-Cards.html" goto :nofile
set PORT=8098
set PAGE=Japanese-SnowMan105-Cards.html
...
echo 서버를 시작합니다 ... %URL%
start "japanese-cards-server" /min %PY% -m http.server %PORT%
timeout /t 2 >nul
start "" "%URL%"
echo.
echo 최소화된 "japanese-cards-server" 창을 닫으면 서버가 종료됩니다.
timeout /t 3 >nul
exit /b 0
...
:nofile
echo [!] Japanese-SnowMan105-Cards.html 을 찾을 수 없습니다.
