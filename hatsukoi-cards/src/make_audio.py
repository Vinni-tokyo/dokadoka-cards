#!/usr/bin/env python3
"""hatsukoi-cards 학습 음원 생성기(edge-tts). 사용: ../../.venv/bin/python make_audio.py"""
...
def study_of(ep=None):
    path = os.path.join(ROOT, 'Japanese-Hatsukoi172-Cards.html')
...
async def gen(ep, force):
    ...
    for k in allkeys:
        ...
        if force or not os.path.exists(os.path.join(adir, fn)):
            todo.append((k, fn))
    print(f'원문 {len(keys)} · 뜻 {len(mkeys)} · 새로 만들 것 {len(todo)}')
