# 도카도카 영어 공부 · Why Social Health Is Key to Happiness and Longevity

YouTube TED 강연 「Why Social Health Is Key to Happiness and Longevity」(Kasley Killam, TED, 2025-03-14, 9분 35초, 조회 112만)로 만든
**영어 학습 카드 앱**(한국어 모어 학습자용). 카드 78장.

신체·정신 건강만큼 «사회적 건강»이 중요하다는 이야기와, 관계를 의도적으로 가꾸는 5-3-1 지침을 소개합니다. 문장이 짧고 또렷해 네 편 중 가장 쉽습니다.
...
간이 서버를 띄우고 브라우저를 엽니다(포트 8087). `file://` 로 열면 YouTube 임베드가 거부되어(Error 153)
재생 버튼이 YouTube 새 탭을 엽니다.

## 기능

- **카드** — 영어 문장 + 한국어 뜻(가리기/보기), 장면별 필터, 체크 진도
- **대본** — 78줄 목록, 줄 클릭 시 그 위치부터 재생, 줄별 구간 반복
- **감상** — 영상을 끊김 없이 재생하며 현재 문장과 앞뒤 문장을 크게 표시
- **학습** — 드릴(간격 반복) · 표현 20 · 단어 20 · 퀴즈 · 오늘 분량 · 학습법
- **장면** — 도입(마야의 이야기) · 사회적 건강이란 · 왜 중요한가 · 5-3-1 지침 · 맺음

## 구성

```
social-cards/
├─ English-Social78-Cards.html   앱 (단일 파일, 데이터 내장)
├─ start.bat / start.sh
├─ README.md
└─ src/
...
- 영상·자막: YouTube `LpSDuDIaBGk` — Why Social Health Is Key to Happiness and Longevity (Kasley Killam, TED, 2025-03-14)
