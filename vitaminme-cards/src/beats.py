...
by = {}
for t, txt in lines:
    if re.search(r'[가-힣]', txt): by.setdefault(txt, []).append(round(bar_pos(t, k), 2))
spread = [max(v) - min(v) for v in by.values() if len(v) > 1]
out = {'bpm': round(bpm, 3), 'beat_interval': round(T, 5), 'offset': round(float(ph), 4), 'phase': k, 'method': 'constant-tempo grid fit',
...
print('繰り返し歌詞の拍位置のばらつき(拍): 最大 %.2f, 平均 %.2f  (0.25以下なら一貫)' % (max(spread), sum(spread)/len(spread)))
