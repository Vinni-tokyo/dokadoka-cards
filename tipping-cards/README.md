# 도카도카 영어 공부 · The Tipping Points of Climate Change — and Where We Stand

YouTube TED 강연 「The Tipping Points of Climate Change — and Where We Stand」(Johan Rockström, TED, 2024-08-15, 18분 36초, 조회 162만)로 만든
**영어 학습 카드 앱**(한국어 모어 학습자용). 카드 157장.

지구 위험 한계선과 티핑포인트를 수치로 짚어 가는 과학 강연입니다. 네 편 중 가장 길고 어휘가 전문적이며, 숫자·단위·비교급 표현이 풍부합니다.
...
간이 서버를 띄우고 브라우저를 엽니다(포트 8085). `file://` 로 열면 YouTube 임베드가 거부되어(Error 153)
재생 버튼이 YouTube 새 탭을 엽니다.

## 기능

- **카드** — 영어 문장 + 한국어 뜻(가리기/보기), 장면별 필터, 체크 진도
- **대본** — 157줄 목록, 줄 클릭 시 그 위치부터 재생, 줄별 구간 반복
- **감상** — 영상을 끊김 없이 재생하며 현재 문장과 앞뒤 문장을 크게 표시
- **학습** — 드릴(간격 반복) · 표현 20 · 단어 20 · 퀴즈 · 오늘 분량 · 학습법
- **장면** — 과학자들이 불안한 이유 · 지구 위험 한계선 · 바다가 흡수한 열 · 티핑포인트 평가 · 지금 위치 · 해법

## 구성

```
tipping-cards/
├─ English-Tipping157-Cards.html   앱 (단일 파일, 데이터 내장)
├─ start.bat / start.sh
├─ README.md
└─ src/
...
- 영상·자막: YouTube `Vl6VhCAeEfQ` — The Tipping Points of Climate Change — and Where We Stand (Johan Rockström, TED, 2024-08-15)
