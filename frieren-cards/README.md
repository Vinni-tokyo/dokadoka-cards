# 도카도카 일본어 공부 · ドカドカ日本語 — 葬送のフリーレン ミニアニメ「●●の魔法」(21편)

TOHO animation 공식 채널이 올리는 『葬送のフリーレン(장송의 프리렌)』공식 미니 애니 **21편(제2~22회, 화당 1~2분, 총 342장)** 으로 만든
**일본어 학습 카드 앱**(한국어 학습자용). 2023년 화제작 본편의 등장인물(프리렌·페른 등)이 짧은 마법 개그를 펼치는 보너스 영상입니다.

- **입구**: `index.html` — 21편 목록. 각 화는 `Japanese-Frieren<회차번호>-Cards.html` 단일 파일.
...
간이 서버를 띄우고 브라우저로 `index.html`(21편 목록)을 엽니다(포트 8088).

## 구성

```
frieren-cards/
├─ index.html                       21편 목록(입구)
├─ Japanese-Frieren2..22-Cards.html      화별 앱(단일 파일, 데이터·음원 내장)
├─ start.bat / start.sh
├─ README.md
└─ src/
   ...
   ├─ kanji.txt        한자 훈음 사전(21편 공용, 409자)
   ├─ words.txt        한자어 사전(21편 공용, 412항목) — gen_words.py + lemma_meanings.py 로 자동 생성
...
python3 gen_words.py > words.txt && for d in ep*/; do cp words.txt kanji.txt $d; done   # 사전 갱신 시
for d in ep*/; do python3 check_cov.py ${d%/}; done   # 한자 커버리지 확인
...
- **자막**: 21편 모두 수동 자막이 없어 YouTube 자동생성 자막(ASR)을 받았습니다. 롤링 캡션 형식이라 한 줄에 여러 문장이
...
- **한자 풀이**: 21편에 나오는 한자 409자·한자어 412항목에 훈음·뜻·한국 한자음을 달았습니다.

## 출처

- 영상: YouTube 공식 채널 TOHO animation 「葬送のフリーレン ミニアニメ「●●の魔法」」 제2~22회(21편).
