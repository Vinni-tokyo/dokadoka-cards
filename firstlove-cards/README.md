...
하위 탭은 **오늘 · 복습 · 목록 · 학습법** 넷이고, 「오늘」은 날짜가 아니라 **단락(1절 · 후렴 1 · 2절 · 후렴 2 · 아웃트로)** 단위로 4단계를 안내합니다.

1. **들으며 읽기** — 노래방 뷰에서 읽기·뜻을 켜고 단락을 한 번 듣기(끝까지 재생하면 자동 완료)
2. **마디 따라 부르기** — 줄마다 그 줄의 마디를 반복(2 › 4 › 8마디, 0.75 › 1×). 「불렀다 · 다음」으로 진행
3. **가사 가리고 부르기** — 뜻 끄기 › 읽기 끄기 › 첫 글자만(어절 첫 글자) 순으로 가리고 부르기
4. **표현 · 단어** — 그 단락의 표현·단어 드릴

마디 반복 패널은 **노래방 뷰 하단**으로 옮겨 한 줄(길이 · 속도 · ◀ ▶ · ■)로 줄였고, ±1마디·±1박 보정은 「미세 조정」 안에 있습니다.
노래방에서 **지금 줄을 누르면 그 줄의 마디가 반복**됩니다. 카드 뷰의 「마디 반복」 버튼은 노래방으로 이동합니다.

## 줄 시각 보정 (2026-09-13)

처음에는 음성 인식 단어 시각만으로 줄을 맞췄는데, 노래는 음을 길게 끌어 인식이 몇 초씩 어긋나는 줄이 있었습니다
(22번 「You are always gonna be my love」 4초 일찍, 25번 「You taught me how」 2초 늦게 시작 등).
영상에 새겨진 일본어 자막(노란색)을 프레임 단위로 추적해 **자막이 뜨고 사라지는 시각**(`src/video_cues.json`)을 얻고,
음성 인식(`src/asr_words.json`)과 합쳐 다시 잡았습니다(`src/retime.py`). 시작은 자막, 끝은 실제 발성 끝을 씁니다.
그래도 어긋나는 줄은 카드의 「편집」에서 ±0.3초씩 조정하거나 `retime.py` 의 규칙을 고치면 됩니다.
