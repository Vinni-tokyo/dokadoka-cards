...
OUT = os.path.join(os.path.dirname(S), 'Japanese-Yubisaki185-Cards.html')
VID = 'B46OizvNlbU'

# 장면 = 이야기의 흐름(장소·시간 전환)으로 나눈 타임라인
# (키, 시작초, 한국어, 일본어)
SCENES = [
 ('intro',  0,      '프롤로그 · 오프닝',       'プロローグ · OP'),
 ('train',  144,    '전철에서의 만남',          '電車での出会い'),
 ('after',  356,    '전철에서 내린 뒤',         '電車を降りてから'),
 ('campus', 425,    '다음 날 아침 · 캠퍼스',    '翌朝 · キャンパス'),
 ('rin',    467,    '린과의 대화',             'りんとの会話'),
 ('night',  614,    '밤, 외출',               '夜の外出'),
 ('bar',    646,    '카페바',                 'カフェバー'),
 ('walk',   937,    '눈 내리는 귀갓길',         '雪の帰り道'),
 ('msg',    1143,   '메시지 · 다음 날',         'メッセージ · 翌日'),
 ('ed',     1285,   '에필로그 · 엔딩',          'エピローグ · ED'),
]
...
SPEAKERS = [
 ...
 ('女子学生', '女子学生',     '여대생',       0),
 ('外国人',   '外国人',       '외국인',       0),
 ('アナウンス', '車内アナウンス', '안내방송',    0),
 ('カーター', 'カーター(映画)', '카터(영화 속)', 0),
 ('ビル',     'ビル(映画)',    '빌(영화 속)',   0),
 ('',         '不明',         '화자 미상',     0),
]
