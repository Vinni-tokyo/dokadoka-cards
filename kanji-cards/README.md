...
**듣기 모드(걸으면서)** · 홈의 「듣기 모드」. 덱·장·모드(단어 듣기 / 거꾸로 듣기 / 한자 듣기)·침묵 길이·바퀴 수·자동 종료를 고르고 시작하면
「읽기 → 침묵(떠올리기) → 뜻 → 읽기 → 대사」 순으로 온라인 음성을 이어서 재생합니다. 침묵도 무음 클립으로 재생하므로 화면이 꺼져도 흐름이 끊기지 않고,
잠금 화면·이어폰 버튼(재생/일시정지/다음/이전)이 됩니다. 「이전」 두 번 또는 「모르겠다」는 그 카드를 복습함에 넣습니다. 들은 카드는 홈의
「오늘 들은 것 다시 보기」에 모여 같은 날 카드 모드로 한 번 더 볼 수 있습니다. 온라인 음성이 막히면 기기 음성으로 바꾸고 화면을 켜 둡니다(Wake Lock).
